...
Personal SDD-enabled repo. Tracking is branch / PR only. **Push the phase branch after every verified phase** so CI runs; PRs still only when the user asks.
...
7. Parent: **one commit per verified phase**, then **push that branch** (CI gate); **PRs only when the user asks**

## Docs

- SDD: `docs/sdd/` (primary: `docs/sdd/distributed-rate-limiter.md` — **Approved** 2026-08-14)
- Stack precursor: `docs/sdd/build-plan-stack.md` (locked choices; plan SoT is the full SDD)
- ADR: `docs/adr/` (0001–0010 **Accepted**)
